...
number_t TOP_LEVEL_usecases_gmm_logsumexp_dps(storage_t stgVar1228, array_number_t arr_dps, card_t arr_shp)
{
	...
	card_t semx_shp = 0;
	env_t_1250 env_t_1250_value = make_empty_env();
	closure_t closure1238 = make_closure(lambda1250, &env_t_1250_value);
	...
	number_t macroDef1249;
	env_t_1251 env_t_1251_value = make_env_t_1251(mx_dps);
	closure_t closure1242 = make_closure(lambda1251, &env_t_1251_value);
	env_t_1252 env_t_1252_value = make_empty_env();
	closure_t closure1245 = make_closure(lambda1252, &env_t_1252_value);
	env_t_1253 env_t_1253_value = make_empty_env();
	closure_t closure1248 = make_closure(lambda1253, &env_t_1253_value);
...
number_t TOP_LEVEL_usecases_gmm_gmm_objective_dps(storage_t stgVar1331, array_array_number_t x_dps, array_number_t alphas_dps, array_array_number_t means_dps, array_array_number_t qs_dps, array_array_number_t ls_dps, number_t wishart_gamma_dps, number_t wishart_m_dps, matrix_shape_t x_shp, card_t alphas_shp, matrix_shape_t means_shp, matrix_shape_t qs_shp, matrix_shape_t ls_shp, card_t wishart_gamma_shp, card_t wishart_m_shp)
{
	...
	number_t macroDef1395;
	card_t n_dps = TOP_LEVEL_linalg_rows_dps(stgVar1332, x_dps, x_shp);
	...
	number_t macroDef1394;
	card_t d_dps = TOP_LEVEL_linalg_cols_dps(stgVar1334, x_dps, x_shp);
	...
	number_t macroDef1376;
	array_number_t macroDef1375 = (array_number_t)stgVar1338;
	...
	for (int i_dps = 0; i_dps < macroDef1375->length; i_dps++)
	{
		...
		number_t macroDef1374;
		array_number_t macroDef1373 = (array_number_t)stgVar1340;
		...
		for (int k_dps = 0; k_dps < macroDef1373->length; k_dps++)
		{
			...
			number_t macroDef1372;
			card_t size1401 = width_card_t(TOP_LEVEL_linalg_vectorSub_shp(x_shp.elem, means_shp.elem));
			array_number_t stgVar1348 = storage_alloc(size1401);
			array_number_t macroDef1371;
			card_t size1400 = width_card_t(ls_shp.elem);
			array_number_t stgVar1347 = storage_alloc(size1400);
			array_number_t macroDef1370;
			card_t size1399 = width_card_t(qs_shp.elem);
			array_number_t stgVar1346 = storage_alloc(size1399);
			array_number_t macroDef1369;
			card_t size1398 = width_card_t(means_shp.elem);
			array_number_t stgVar1352 = storage_alloc(size1398);
			array_number_t macroDef1368;
			card_t size1397 = width_card_t(x_shp.elem);
	...
	array_number_t macroDef1392 = (array_number_t)stgVar1356;
	...
	{
		env_t_1405 env_t_1405_value = make_empty_env();
		closure_t closure1379 = make_closure(lambda1405, &env_t_1405_value);
		...
		number_t macroDef1390;
		card_t size1408 = width_card_t(qs_shp.elem);
		array_number_t stgVar1360 = storage_alloc(size1408);
		array_number_t macroDef1386;
		env_t_1406 env_t_1406_value = make_empty_env();
		closure_t closure1382 = make_closure(lambda1406, &env_t_1406_value);
		env_t_1407 env_t_1407_value = make_empty_env();
		closure_t closure1385 = make_closure(lambda1407, &env_t_1407_value);
		...
		env_t_1409 env_t_1409_value = make_empty_env();
		closure_t closure1389 = make_closure(lambda1409, &env_t_1409_value);
...
void TOP_LEVEL_usecases_gmm_test_gmm_dps(storage_t stgVar1415, array_number_t dum_dps, card_t dum_shp)
{
	...
	int stgVar1420_offsetVar = 0;
	storage_t stgVar1421 = STG_OFFSET(stgVar1420, MATRIX_HEADER_BYTES(3) + stgVar1420_offsetVar);
	array_number_t macroDef1434 = (array_number_t)stgVar1421;
	...
	macroDef1437->arr[0] = macroDef1434;
	;
	stgVar1420_offsetVar += VECTOR_ALL_BYTES(macroDef1437->arr[0]->length);
	storage_t stgVar1425 = STG_OFFSET(stgVar1420, MATRIX_HEADER_BYTES(3) + stgVar1420_offsetVar);
	array_number_t macroDef1435 = (array_number_t)stgVar1425;
	...
	macroDef1437->arr[1] = macroDef1435;
	;
	stgVar1420_offsetVar += VECTOR_ALL_BYTES(macroDef1437->arr[1]->length);
	storage_t stgVar1429 = STG_OFFSET(stgVar1420, MATRIX_HEADER_BYTES(3) + stgVar1420_offsetVar);
	array_number_t macroDef1436 = (array_number_t)stgVar1429;
	...
	;
	stgVar1420_offsetVar += VECTOR_ALL_BYTES(macroDef1437->arr[2]->length);
	;
