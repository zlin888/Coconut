...
array_number_t TOP_LEVEL_usecases_ba_project_dps(storage_t stgVar299, array_number_t cam_dps, array_number_t x_dps, card_t cam_shp, card_t x_shp)
{
	...
	array_number_t macroDef471;
	array_number_t Xcam270_dps = 0;
	if ((sqtheta212_dps) != (0))
	{
		...
		array_number_t macroDef464;
		array_number_t macroDef460 = (array_number_t)stgVar316;
		...
		macroDef460->arr[0] = (((xi217_dps) * (theta_inv216_dps)) * ((x218_dps) - (y219_dps))) - (((xi220_dps) * (theta_inv216_dps)) * ((x221_dps) - (y222_dps)));
		;
		number_t xi223_dps = cam_dps->arr[2];
		...
		macroDef460->arr[1] = (((xi223_dps) * (theta_inv216_dps)) * ((x224_dps) - (y225_dps))) - (((xi226_dps) * (theta_inv216_dps)) * ((x227_dps) - (y228_dps)));
		;
		number_t xi229_dps = cam_dps->arr[0];
	...
	else
	{
		...
		array_number_t macroDef468;
		array_number_t macroDef465 = (array_number_t)stgVar389;
		...
		macroDef465->arr[0] = ((cam_dps->arr[1]) * ((x253_dps) - (y254_dps))) - ((cam_dps->arr[2]) * ((x255_dps) - (y256_dps)));
		;
		number_t x257_dps = x_dps->arr[0];
		...
		macroDef465->arr[1] = ((cam_dps->arr[2]) * ((x257_dps) - (y258_dps))) - ((cam_dps->arr[0]) * ((x259_dps) - (y260_dps)));
		;
		number_t x261_dps = x_dps->arr[1];
